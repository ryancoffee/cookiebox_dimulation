...
def getfilenames(globstring):
    return glob.glob(globstring)

def main():
    globstring = './data_fs/raw/CookieBox_waveforms.4pulses.image00*.dat'
    if len(sys.argv)>1:
        globstring = sys.argv[1]
        print(globstring)
    fnameslist = glob.glob(globstring)
    print(len(fnameslist))
    for filename in fnameslist:
